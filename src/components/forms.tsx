...
export default function Form() {
    ...
    // Watch when the section comes into view
    const sectionRef = useRef<HTMLDivElement>(null)
    const isInView = useInView(sectionRef, { once: false, margin: '-100px' }) // margin to trigger earlier
    ...
    const successVariants = {
        ...
        exit: { scale: 0.8, opacity: 0, transition: { ease: "easeIn" as const } }
    }
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2">
                            ...
                            >
                                ...
                                <AnimatePresence mode="wait">
                                    {status === 'idle' || status === 'submitting' ? (
                                        <motion.div key="form-fields" variants={containerVariants} className="space-y-6">
                                            {/* Name */}
                                            <motion.div variants={itemVariants}>
                                            ...
                                            <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-300 mb-1">Services Needed</label>
                                                    <input
                                                        type="text"
                                                        name="services"
                                        ...
                                        <motion.div key="success-message" variants={successVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col items-center justify-center text-center p-8">
                                            <CheckCircle className="h-16 w-16 text-green-500 mb-4" />
                                            <h3 className="text-2xl font-bold text-white mb-2">Message Sent!</h3>
                                            <p className="text-gray-300 mb-6">Thank you for contacting us. Our team will get back to you shortly.</p>
                                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setStatus('idle')} className="px-6 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors duration-200">
                                                Send Another Message
                                            </motion.button>
                                        </motion.div>
                                    ) : (
                                        <motion.div key="error-message" variants={successVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col items-center justify-center text-center p-8">
                                            <XCircle className="h-16 w-16 text-red-500 mb-4" />
                                            <h3 className="text-2xl font-bold text-white mb-2">Submission Failed</h3>
                                            <p className="text-gray-300 mb-6">There was an error sending your message. Please try again later.</p>
                                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setStatus('idle')} className="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors duration-200">
                                                Try Again
                                            </motion.button>
                                        </motion.div>
                                    )}
                        ...
                        >
                            ...
                            <div className="relative z-10 text-center">
                                ...
                                <motion.div variants={itemVariants} className="text-gray-300 max-w-md mx-auto">
                                    <p className="mb-4">Our team of experts is ready to help you create exceptional digital experiences.</p>
                                    <p>Whether you need web development, design, or consultation, we&#39;ve got you covered.</p>
                                </motion.div>
